...
Three implementations of the :class:`~ltx_audio_pipeline.utils.types.Denoiser` protocol:
...
all guidance passes into a single transformer call. In the audio-only package,
the transformer may be a thin AV-shaped adapter around the underlying
single-stream audio model.
"""

import torch

from ltx_audio_core.components.guiders import MultiModalGuider, MultiModalGuiderFactory, MultiModalGuiderParams
from ltx_audio_core.guidance.perturbations import (
...
from ltx_audio_pipeline.utils.helpers import modality_from_latent_state
from ltx_audio_pipeline.utils.types import DenoisedLatentResult, TransformerLike
...
def _guided_denoise(  # noqa: PLR0913,PLR0915
    ...
    *,
    ...
    with the pipeline's batch-splitting adapter, the transformer may split
    this batch into sequential chunks internally.
