"""Diffusion samplers for the audio pipeline.

The public function signatures intentionally keep the AV-shaped state/denoiser
interface from the main pipeline so this package remains easy to merge back.
In audio-only usage, callers pass ``video_state=None`` and rely on the
audio-only transformer adapter behind the shared sampler surface.
"""
...
from ltx_audio_core.components.diffusion_steps import EulerCfgPpDiffusionStep, Res2sDiffusionStep
from ltx_audio_core.components.protocols import DiffusionStepProtocol
from ltx_audio_core.utils import to_denoised, to_velocity
from ltx_audio_pipeline.utils.helpers import post_process_latent, timesteps_from_mask
from ltx_audio_pipeline.utils.res2s import get_res2s_coefficients
from ltx_audio_pipeline.utils.types import Denoiser, LatentState, TransformerLike
...
def euler_denoising_loop(
    ...
    denoiser:
        ...
        and must return a :class:`~ltx_audio_pipeline.utils.types.DenoisedLatentResult`.
    ### Returns
    tuple[LatentState | None, LatentState | None]
...
def euler_cfg_pp_denoising_loop(
    ...
    Requires a guided denoiser whose :class:`~ltx_audio_pipeline.utils.types.DenoisedLatentResult`
    ...
    video_state:
        Current video :class:`~ltx_audio_core.types.LatentState`, or ``None``.
    audio_state:
        Current audio :class:`~ltx_audio_core.types.LatentState`, or ``None``.
    stepper:
        :class:`~ltx_audio_core.components.diffusion_steps.EulerCfgPpDiffusionStep`
        instance carrying ``eta`` and ``s_noise`` parameters.
    transformer:
        The diffusion model passed to the denoiser at each step.
    denoiser:
        Callable implementing :class:`~ltx_audio_pipeline.utils.types.Denoiser`.
    noise_seed:
