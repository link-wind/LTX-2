...
class Modality:
    """
    Input data for the audio stream in the transformer.
    ...
    Attributes:
        ...
        positions: Per-token patch coordinates used to build the RoPE
            ...
            shape is ``(B, n_pos_dims, T, 2)`` where ``n_pos_dims=1`` for
            audio time positions; the last dim of size 2 holds the ``[start, end)``
        ...
        attention_mask: Optional 2-D self-attention mask, shape ``(B, T, T)``.
            ...
            :class:`~ltx_audio_core.conditioning.types.attention_strength_wrapper.ConditioningItemAttentionStrengthWrapper`.
    """
    ...
    # n_pos_dims=1 for audio; last dim holds [start, end) patch bounds.
