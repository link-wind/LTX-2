...
from ltx_audio_core.block_streaming.disk import DiskBlockReader, DiskTensorReader, LoraSource
...
from ltx_audio_core.block_streaming.source import DiskWeightSource, PinnedWeightSource, WeightSource
from ltx_audio_core.block_streaming.utils import allocate_layout_views, derive_layout, make_block_key, resolve_attr
from ltx_audio_core.block_streaming.wrapper import BlockStreamingWrapper
from ltx_audio_core.loader.fuse_loras import FuseRule, bf16_fuse_rule, fuse_lora_weights
from ltx_audio_core.loader.helpers import create_meta_model, load_state_dict, read_model_config
from ltx_audio_core.loader.module_ops import ModuleOps
from ltx_audio_core.loader.primitives import (
...
from ltx_audio_core.loader.sft_loader import SafetensorsModelStateDictLoader
from ltx_audio_core.model.model_protocol import ModelConfigurator, ModelType
...
class StreamingModelBuilder(Generic[ModelType], ModelBuilderProtocol[ModelType]):
    ...
    def build(
        ...
        **_kwargs: object,
        ...
        block_key_map = _filter_block_key_map_for_model(block_key_map, blocks)
...
def _filter_block_key_map_for_model(
    block_key_map: dict[int, list[tuple[str, str]]],
    ...
    for block_idx, entries in block_key_map.items():
        ...
        valid_param_names = set(dict(blocks[block_idx].named_parameters()).keys())
        kept_entries = [(sft_key, param_name) for sft_key, param_name in entries if param_name in valid_param_names]
        dropped += len(entries) - len(kept_entries)
        if kept_entries:
            filtered[block_idx] = kept_entries

    if dropped:
        logger.info(
            "Filtered %d block checkpoint entries that are not present in the streaming meta model",
            dropped,
        )
