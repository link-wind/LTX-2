...
from ltx_audio_core.block_streaming.disk import LoraSource
from ltx_audio_core.block_streaming.pool import WeightPool
from ltx_audio_core.block_streaming.source import WeightSource
from ltx_audio_core.loader.fuse_loras import FuseRule, aggregate_lora_products, bf16_fuse_rule
...
class WeightsProvider:
    ...
    def _fuse_block_loras(self, idx: int, weights: dict[str, torch.Tensor]) -> None:
        for name, tensor in weights.items():
            ...
            products = (
                ab
                for ab in (s.get_ab(prefix, device=self._target_device, dtype=self._fuse_rule.aggregation_dtype) for s in self._lora_sources)
                if ab is not None
            )
            deltas = aggregate_lora_products(products, self._fuse_rule.aggregation_dtype)
            if deltas is None:
                continue
