...
from ltx_audio_core.conditioning.mask_utils import update_attention_mask
...
class AudioConditionByReferenceLatent:
    ...
    The reference tokens are appended so the target audio tokens stay in the
    first ``num_noisy_tokens`` positions and can be kept by
    :meth:`ltx_audio_core.tools.LatentTools.clear_conditioning`.
    Args:
