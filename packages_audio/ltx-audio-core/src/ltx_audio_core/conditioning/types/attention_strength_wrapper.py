...
class ConditioningItemAttentionStrengthWrapper(ConditioningItem):
    ...
    Args:
        ...
        attention_mask: Per-token attention weight controlling how strongly the
            ...
            for per-token control, where ``M`` is the number of appended
            conditioning tokens. Values in ``[0, 1]``.
    Example::
        cond = ConditioningItemAttentionStrengthWrapper(
            AudioConditionByReferenceLatent(patchified=ref_tokens, positions=ref_positions, strength=1.0),
            attention_mask=0.5,
        )
